...
# modified # import skimage.filters as filters
import gzip
import os
import Queue
...
#modified
num_train = 100 # 61578 # 70948
num_test = 36 # 79975 # 79971
...
def chunk_lcn(chunk, sigma_mean, sigma_std, std_bias=0.0, rescale=1.0):
    ...
    for k in xrange(len(chunk)):
        means[k] = filters.gaussian_filter(chunk[k], sigma_mean, multichannel=True)
    ...
    for k in xrange(len(chunk)):
        variances[k] = filters.gaussian_filter(chunk_squared[k], sigma_std, multichannel=True)
