...
def realtime_augmented_data_gen(data_indices,
                                batch_size=BATCH_SIZE, augmentation_params=default_augmentation_params,
    ...
    while True:
        if n*batch_size >= len(data_indices):
            # print "DEBUG: DATA GENERATION COMPLETED"
            break

        # start_time = time.time()
        # selected_indices = select_indices(num_train, chunk_size)
        selected_indices = range(n*batch_size, min((n+1)*batch_size, len(data_indices)))
        labels = y_train[data_indices[selected_indices]]

        process_func = processor_class(ds_transforms, augmentation_params, target_sizes)

        target_arrays = [np.empty((len(selected_indices), size_x, size_y, 3), dtype='float32') for size_x, size_y in target_sizes]
        ...
        yield target_arrays, len(selected_indices)
